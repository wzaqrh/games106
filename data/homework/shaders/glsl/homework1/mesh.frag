...
layout(push_constant) uniform PushConsts {
	...
	layout(offset = 76) float g;
	layout(offset = 80) float b;
} material;
...
layout (set = 1, binding = 1) uniform UBOShared {
	vec4 lights[4];
} uboParams;

layout (set = 2, binding = 2) uniform sampler2D samplerColorMap;
...
void main() 
{
	vec4 albedo = texture(samplerColorMap, inUV) * vec4(inColor, 1.0);
